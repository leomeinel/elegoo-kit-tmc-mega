...
constexpr uint8_t RGB_LED_RED = 4;
constexpr uint8_t RGB_LED_GREEN = 5;
...
void setup()
{
  constexpr uint8_t OUTPUT_PINS[3] = { RGB_LED_RED, RGB_LED_GREEN, RGB_LED_BLUE };
...
#pragma unroll
  for (const int (&color)[3] : HTML_COLORS)
  {
    ...
    constexpr uint32_t EXEC_LOOP_DELAY = 1000;
    delay(EXEC_LOOP_DELAY);
  }
}
